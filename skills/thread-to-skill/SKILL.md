...
- 每次使用本 skill 写入知识库前，先检查项目根目录是否存在 `thread-to-skill-knowledge.config.json`。
...
- 配置文件中的 `docsPath` 表示知识库文档根目录；所有 `source-records/`、`review-records/`、`domains/`、`skill-candidates/`、`skills/` 都必须写到 `<project-root>/<docsPath>/` 下。
- 如果配置文件存在但 JSON 不合法，或缺少 `docsPath`，不要自动修复或覆盖；先停下来说明问题，让用户决策。
...
- 用户要求“沉淀”“复盘并写入”“维护知识库”“整理成长期规则”时，必须按 `source-records/`、`review-records/`、`domains/`、`skill-candidates/`、`skills/` 的分层职责落地，不能只生成 1-2 个临时总结文档。
- `review-records/<review-id>/` 是每次复盘的必需层，至少包含 `README.md`、`extracted-rules.md`、`high-confidence.md`、`needs-confirmation.md`、`conflicts.md`。即使某一类为空，也要保留文件并写明“暂无”。
- `source-records/<source-id>/` 只在需要保留来源证据或脱敏上下文时创建；一旦创建，必须包含 `README.md`、`evidence-summary.md`、`redaction-notes.md`，不能只放一个来源摘要。
- `domains/<domain>/` 只保存已稳定、可复用的最终态规则。高置信度规则必须合并到对应领域文档；中低置信度和冲突项不能直接进入领域层。
- `skill-candidates/` 和 `skills/` 只有在成熟度检测和用户确认后才创建或更新。
- 如果用户指定了一个输出目录，也要在该目录下创建上述分层子目录，而不是把所有内容平铺成两个 md 文件。
...
复盘 thread 时，以下结构写入 `review-records/<review-id>/README.md`；不要把它作为唯一产物来替代分层文件：
...
`review-records/<review-id>/extracted-rules.md` 使用以下结构：
...
`review-records/<review-id>/high-confidence.md` 使用以下结构：
...
`review-records/<review-id>/needs-confirmation.md` 使用以下结构：
...
```md
<!-- 写入 review-records/<review-id>/conflicts.md -->
...
- `review-records/<review-id>/README.md`、`extracted-rules.md`、`high-confidence.md`、`needs-confirmation.md`、`conflicts.md` 是否齐全。
- 是否没有用 1-2 个平铺 md 文件替代分层目录结构。
- 是否所有新增内容都写入 `thread-to-skill-knowledge.config.json` 中 `docsPath` 指向的目录。
- 是否没有在项目根目录误创建 `thread-to-skill-knowledge/` 目录。
